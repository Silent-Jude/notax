...
export default {
  ...
  methods: {
    ...
    clear() {
      ...
      // 表单数据
      this.form.billId = ''
      this.form.endRepealDate = ''
      this.form.StorageId = ''
      this.form.startRepealDate = ''
    ...
    isdateChange(val) {
      if (val) {
        var time = new Date()
        this.form.startRepealDate = `${time.getFullYear()}-${time.getMonth() + 1}-1`
        // 设置当前时间
        this.form.endRepealDate = `${time.getFullYear()}-${time.getMonth() + 1}-${time.getDate()}`
      } else {
        this.form.startRepealDate = ''
        this.form.endRepealDate = ''
      }
