...
export default {
  ...
  methods: {
    ...
    clear() {
      ...
      // 表单数据
      this.form.billId = ''
      this.form.endRequestDate = ''
      this.form.agencyId = ''
      this.form.startRequestDate = ''
    ...
    isdateChange(val) {
      if (val) {
        var time = new Date()
        this.form.startRequestDate = `${time.getFullYear()}-${time.getMonth() + 1}-1`
        // 设置当前时间
        this.form.endRequestDate = `${time.getFullYear()}-${time.getMonth() + 1}-${time.getDate()}`
      } else {
        this.form.startRequestDate = ''
        this.form.endRequestDate = ''
      }
