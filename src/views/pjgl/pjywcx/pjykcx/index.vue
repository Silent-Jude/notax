...
export default {
  ...
  computed: {
    formPrintData() {
      return [
        ...
        { text: '仓库名称', value: this.printingname, checked: true },
  ...
  methods: {
    ...
    clear() {
      ...
      // 表单数据
      this.form.billId = ''
      this.form.endMoveDate = ''
      this.form.StorageId = ''
      this.form.startMoveDate = ''
    ...
    isdateChange(val) {
      if (val) {
        var time = new Date()
        this.form.startMoveDate = `${time.getFullYear()}-${time.getMonth() + 1}-1`
        // 设置当前时间
        this.form.endMoveDate = `${time.getFullYear()}-${time.getMonth() + 1}-${time.getDate()}`
      } else {
        this.form.startMoveDate = ''
        this.form.endMoveDate = ''
      }
