...
export default {
  ...
  methods: {
    ...
    isdateChange(val) {
      if (val) {
        var time = new Date()
        this.form.startApplyDate = `${time.getFullYear()}-${time.getMonth() + 1}-1`
        // 设置当前时间
        this.form.endApplyDate = `${time.getFullYear()}-${time.getMonth() + 1}-${time.getDate()}`
      } else {
        this.form.startApplyDate = ''
        this.form.endApplyDate = ''
      }
