...
export default {
  ...
  methods: {
    ...
    isdateChange(val) {
      if (val) {
        var time = new Date()
        this.form.startStorageDate = `${time.getFullYear()}-${time.getMonth() + 1}-1`
        // 设置当前时间
        this.form.endStoragrDate = `${time.getFullYear()}-${time.getMonth() + 1}-${time.getDate()}`
      } else {
        this.form.startStorageDate = ''
        this.form.endStoragrDate = ''
      }
