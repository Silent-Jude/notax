<!-- fssrybjkszfywcx 非税收入一般缴款书作废业务查询-->
<template>
...
export default {
  ...
  methods: {
    ...
    selectdwmc(val) {
      ...
      this.form.invalidAgencyId = val.currentRow.guid
    ...
    clear() {
      ...
      // 表单数据
      this.form.billId = ''
      this.form.endInvalidDate = ''
      this.form.invalidAgencyId = ''
      this.form.startInvalidDate = ''
    ...
    isdateChange(val) {
      if (val) {
        var time = new Date()
        this.form.startInvalidDate = `${time.getFullYear()}-${time.getMonth() + 1}-1`
        // 设置当前时间
        this.form.endInvalidDate = `${time.getFullYear()}-${time.getMonth() + 1}-${time.getDate()}`
      } else {
        this.form.startInvalidDate = ''
        this.form.endInvalidDate = ''
      }
