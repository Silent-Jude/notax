...
export default {
  ...
  methods: {
    ...
    selectdwmc(val) {
      ...
      this.form.applyAgencyId = val.currentRow.guid
    ...
    clear() {
      ...
      this.form.track = ''
      this.form.endDestoryDate = ''
      this.form.startDestoryDate = ''
    ...
    isdateChange(val) {
      if (val) {
        var time = new Date()
        this.form.startDestoryDate = `${time.getFullYear()}-${time.getMonth() + 1}-1`
        // 设置当前时间
        this.form.endDestoryDate = `${time.getFullYear()}-${time.getMonth() + 1}-${time.getDate()}`
      } else {
        this.form.startDestoryDate = ''
        this.form.endDestoryDate = ''
      }
