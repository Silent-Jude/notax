...
<template>
  <el-scrollbar style="height:100%;width:100%;">
    <div class="pjyztzs">
          ...
          <el-col :span="3">
            <el-form-item label="" label-width="0px">
              <el-select :disabled="!form.isfilter" v-model="form.filterName" placeholder="">
                ...
                <el-option label="退回日期" value="backdate"/>
                <el-option label="制单人" value="operator"/>
                <el-option label="审核人" value="reviewer"/>
                <el-option label="审核日期" value="auditdate"/>
                <el-option label="退回单位编码" value="thagencycode"/>
                <el-option label="退回单位名称" value="thagencyname"/>
                <el-option label="票据准用编码" value="permitno"/>
                <el-option label="票据准用名称" value="billpermitname"/>
                <el-option label="入库仓库编码" value="storagecode"/>
                <el-option label="入库仓库名称" value="storagename"/>
      ...
      <!-- 添加对话框 -->
      <add ref="layer1" :form="currentRowState" title="财政票据退回"/>
      <!-- 请选择票据种类信息 -->
...
export default {
  ...
  data() {
    return {
      ...
      addLayerForm: {
        backdate: formatDate(new Date()), // 业务日期
        receipno: '', // 单据编码
        operator: '', // 制单人
        delivered: '', // 送审人
        reviewer: '', // 审核人
  ...
  methods: {
    ...
    dbRow(row) {
      ...
      this.$refs.layer1.submitData.back = this.currentRow
      this.$refs.layer1.submitData.bilBackDetails.bilBackDetail = this.currentRow.details

      this.$refs.layer1.selectedBasChargeAgency = {}
      this.$refs.layer1.selectedBasBillStorage = {}

      this.$refs.layer1.backAgencyName = this.currentRow.thagencyname
      this.$refs.layer1.backBillStorageName = this.currentRow.storagename

      this.$refs.layer1.selectedBasChargeAgency.perimitguid = this.currentRow.permitid
      this.$refs.layer1.selectedBasChargeAgency.agencyid = this.currentRow.agencyid
    ...
    billUpdate() {
      ...
      if (this.currentRow.auditmark === '1') {
        ...
        // this.$refs.layer1.submitData.back.guid = this.currentRow.guid
    ...
    add() {
      ...
      this.$refs.layer1.backAgencyName = ''
      this.$refs.layer1.backBillStorageName = ''
