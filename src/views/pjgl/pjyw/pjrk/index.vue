...
<template>
  <el-scrollbar style="height:100%;width:100%;">
    <div class="pjyztzs">
          ...
          <el-col :span="3">
            <el-form-item label="" label-width="0px">
              <el-select :disabled="!form.isfilter" v-model="form.filterName" placeholder="">
                ...
                <el-option label="入库日期" value="storagedate"/>
                ...
                <el-option label="入库类型 " value="entryvalue"/>
      ...
      <!-- 添加票据入库对话框 -->
      <add ref="layer1" :form="currentRowState" title="票据入库"/>
      <!-- 请选择票据种类 -->
      ...
      <!-- <tablePrintSet :template-data="templateDataList"/> -->
...
export default {
      ...
      templateInitData: { //  add
        ...
        templatecontent: {
          ...
          tableHead: [
            ...
            { text: '入库日期', key: 'storagedate', width: '170', checked: true },
            { text: '审核标志', key: 'auditmark', width: '90', checked: true },
            { text: '审核人', key: 'reviewer', width: '80', checked: true },
            { text: '审核日期', key: 'auditdate', width: '170', checked: true },
            { text: '仓库编码', key: 'storagecode', width: '90', checked: true },
            { text: '仓库名称', key: 'storagename', width: '200', checked: true },
            { text: '入库类型', key: 'entryvalue', width: '90', checked: true },
            { text: '经手人', key: 'operator', width: '90', checked: true }
          ],
          printFoot: [
            // { text: '票据号码', value: '', checked: true },
            // { text: '制表人', value: this.$store.state.user.name, checked: true },
  ...
  methods: {
    ...
    dbRow() {
      ...
      this.$refs.layer1.submitData.storage = this.currentRow
      this.$refs.layer1.submitData.bilStorageDetails.bilStorageDetail = this.currentRow.details
      // this.$refs.layer1.submitData.storage.guid = this.currentRow.guid
      this.$refs.layer1.selectedBasBillPrintingList = []
      this.$refs.layer1.selectedMoveInBillStorageList = []

      // add表单中的移出，移入单位初始仓库名称。
      this.$refs.layer1.printUnit = this.currentRow.printingname
      this.$refs.layer1.moveInBillStorage = this.currentRow.zrstoragename
      // add表单中的移出，移入单位初始仓库id。
      this.$refs.layer1.selectedBasBillPrintingList.guid = this.currentRow.printingid
      this.$refs.layer1.selectedMoveInBillStorageList.guid = this.currentRow.storeid
    ...
    billUpdate() {
      if (this.currentRow.auditmark === '1') {
        ...
        // add表单中的移出，移入单位初始仓库名称。
        this.$refs.layer1.printUnit = this.currentRow.printingname
        this.$refs.layer1.moveInBillStorage = this.currentRow.storagename
    ...
    handleCurrentChangedetails(val) {
      ...
      console.log('当前行为：', this.currentRow)
    },
    // 关闭返回首页
    close() {
    ...
    add() {
      ...
      this.$refs.layer1.moveInBillStorage = ''
