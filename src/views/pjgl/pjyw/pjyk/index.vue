...
<template>
  <el-scrollbar style="height:100%;width:100%;">
    <div class="pjyztzs">
          ...
          <el-col :span="3">
            <el-form-item label="" label-width="0px">
              <el-select :disabled="!form.isfilter" v-model="form.filterName" placeholder="">
                ...
                <el-option label="移库日期" value="movedate"/>
                <el-option label="经手人" value="attn"/>
                <el-option label="审核人" value="reviewer"/>
                <el-option label="审核日期" value="auditdate"/>
                <el-option label="移出仓库编码" value="zcstoragecode"/>
                <el-option label="移出仓库名称 " value="zcstoragename"/>
                <el-option label="移入仓库编码 " value="zrstoragecode"/>
                <el-option label="移入仓库名称 " value="zrstoragename"/>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="4">
      ...
      <!-- 添加票据移库对话框 -->
      <add ref="layer1" :form="currentRowState" title="票据移库"/>
      <!-- 请选择票据种类 -->
...
export default {
  ...
  data() {
    return {
      ...
      addLayerForm: {
        movedate: formatDate(new Date()), // 业务日期
        receipno: '', // 单据编码
        operator: '', // 制单人
        delivered: '', // 送审人
        reviewer: '', // 审核人
  ...
  methods: {
    ...
    dbRow() {
      ...
      this.$refs.layer1.submitData.moveRecord = this.currentRow
      this.$refs.layer1.submitData.bilMoveRecordDetails.bilMoveRecordDetail = this.currentRow.details
      // this.$refs.layer1.submitData.moveRecord.guid = this.currentRow.guid
      this.$refs.layer1.selectedMoveOutBillStorageList = []
      this.$refs.layer1.selectedMoveInBillStorageList = []

      // add表单中的移出，移入单位初始仓库名称。
      this.$refs.layer1.moveOutBillStorage = this.currentRow.zcstoragename
      this.$refs.layer1.moveInBillStorage = this.currentRow.zrstoragename
      // add表单中的移出，移入单位初始仓库id。
      this.$refs.layer1.selectedMoveOutBillStorageList.guid = this.currentRow.outstorecodeid
      this.$refs.layer1.selectedMoveInBillStorageList.guid = this.currentRow.storageid
    ...
    add() {
      ...
      this.$refs.layer1.selectedMoveInBillStorageList = []
      // add表单中的移出，移入单位初始仓库名称。
      this.$refs.layer1.moveOutBillStorage = ''
