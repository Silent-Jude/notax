  ...
  <div class="dwlrdzsq">
    ...
    <el-form ref="form" :model="form" size="mini" label-width="80px">
        ...
        <el-col :span="2">
          <el-form-item label-width="20px" label="">
            <el-checkbox-group v-model="form.isdate" @change="isdateChange">
              <el-checkbox label="收缴日期" name="type"/>
            </el-checkbox-group>
          </el-form-item>
        </el-col>
        <el-col :span="3">
      ...
      <el-row> -->
        ...
        <el-col :span="3">
          <el-form-item label="" label-width="0px">
            <el-select v-model="form.cboFilter" placeholder="">
    ...
    <div class="wrap">
      <el-scrollbar style="height:100%;width:100%;">
        <el-table
          :data="currentRow ? currentRow.oldetails :[]"
          border
          style="width: 100%">
          <el-table-column
            type="index"/>
    ...
    <div class="wrap">
      <el-scrollbar style="height:100%;width:100%;">
        <el-table
          :data="currentRow ? currentRow.newetails:[]"
          border
          style="width: 100%">
          <el-table-column
            type="index"/>
...
export default {
  ...
  data() {
    return {
      ...
      //   'czbillSendAudit', 'tablePrintPreview', 'print', 'set', 'export', 'tablePrintSet'],
      // 当前表单数据
      form: {
        // 页码
        pageIndex: 1,
  ...
  methods: {
    postBillAudit: fssrtzgl.billAudit,
    // 审核 对话框
    billAudit() {
    ...
    billAuditType(type) {
      ...
      this.postBillAudit(params).then(res => {
        ...
        fssrtzgl.get(this.currentRow.guid).then(res => {
          ...
          // 设置数据
          this.$refs.layer1.currentRow = res.data[0]
          // 设置编辑状态
    ...
    dbRow(row) {
      console.log('row', row)
      fssrtzgl.get(row.guid).then(res => {
        console.log(res)
        console.log(JSON.stringify(res.data))
        // 显示add组件
        this.$refs.layer1.dialogTableVisible = true
    ...
    add() {
      ...
      this.$refs.layer1.currentRow.newDetails = []
      // 设置编辑状态
      this.$refs.layer1.state = true
    },
    // 修改
    ...
    billDelete() {
      this.$confirm('您确定要删除吗？', '提示', {
        ...
        // 删除
        fssrtzgl.remove(this.currentRow.guid).then(res => {
          this.$message({
            type: 'success',
            message: '删除成功!'
          })
    ...
    onSubmit() {
      // list数据
      fssrtzgl.list(this.form).then(res => {
    ...
    billSendAudit() {
      this.$confirm('您确定要送审吗？', '提示', {
        ...
        fssrtzgl.sendReported(this.currentRow.guid).then(res => {
          // 在add组件内时
          fssrtzgl.get(this.currentRow.guid).then(res => {
            console.log(res)
            console.log(JSON.stringify(res.data))
