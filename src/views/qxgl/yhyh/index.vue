...
import { getTree, getDetail, getTableListInfo, pwdReset, batchCreateAdmUser } from '@/api/jczl'
...
export default {
  ...
  data() {
    return {
      ...
      clientButnsArray: ['close', 'cz', 'save', 'abolish', 'refresh', 'update', 'add', 'delete', 'tablePrintPreview', 'tablePrint', 'tablePrintSet', 'tablePrintExport', 'batchAdd'], // 这里填写那些服务器没有返回，但是却在按钮组中存在的按钮。
      urlPath: '/sys/user',
      refreshTree: true,
      data: []
    }
  ...
  methods: {
    ...
    getAllGuid() {
      var obj = this.$store.state.app.selectedNodeInfo
      var guid = []
      if
      // (obj.id === '-1') {
      //   return ''
      ...
      (!obj.hasChildren) {
        ...
        this.traverse(obj, guid)
      }
      if (guid.length !== 1) {
        return guid.join()
      } else {
        return guid.join().replace(/'/g, '')
      }
    },
    // 自定义事件，用来接受treeComponents触发的获取详细信息事件。
    getDetailRequest(params) {
      console.log('ps', this.getAllGuid())

      params = { guid: this.getAllGuid() }
      // this.getAllGuid(params)
