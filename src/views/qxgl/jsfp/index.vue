...
<template>
  <div>
    ...
    <TreeLayout v-if="refreshTree" :data="leftTreeData">
      ...
      <div slot="tabs" class="tree-menu el-tabs__content">
...
import leftLocate from '../../xtgl/sjqx/components/leftLocate'
import rightLocate from '../../xtgl/sjqx/components/rightLocate'
import TreeLayout from '../../xtgl/sjqx/components/TreeLayout'
import TabList from '../../xtgl/sjqx/components/TabList'
import toolBar from '@/components/toolBar'
import listInfo from '../../xtgl/sjqx/components/listInfo'
...
export default {
  ...
  data() {
    return {
      ...
      clientButnsArray: ['close', 'grant', 'grantSave', 'grantAbolish', 'refresh1'], // 这里填写那些服务器没有返回，但是却在按钮组中存在的按钮。
      // listInfo的表头，需要手动传进去。
  ...
  created() {
    ...
    Bus.$off('hasChildren')
    Bus.$on('hasChildren', (hasChildren) => {
      this.hasChildren = hasChildren
    })

    Bus.$off('clearRight')
    Bus.$on('clearRight', () => {
      this.$store.commit('SET_SELECTEDLEFTTREENODEGUID', '')
      // SET_SELECTEDLEFTTREENODEGUID
      this.rightTreeData = []
    })
  ...
  methods: {
    ...
    getRightTreeInfo(params) {
      ...
      if (this.exchangeState === 'normal') {
        this.getTreer({ [this.rightTreeParams]: params.guid, hasChildren: this.hasChildren }).then(res => {
        ...
        this.getTreer({ [this.leftTreeParams]: params.guid, hasChildren: this.hasChildren }).then(res => {
