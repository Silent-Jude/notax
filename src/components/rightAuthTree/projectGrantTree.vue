...
export default {
  ...
  computed: {
    ...mapState({
      selectedBillRowGuid: state => state.app.selectedBillRowGuid,
  ...
  created() {
    ...
    // 获取右侧树详情信息
    Bus.$off('getProjectGrantTree')
    Bus.$on('getProjectGrantTree', (params) => {
      this.getProjectGrantTree(params)
    })
  ...
  methods: {
    ...
    getProjectGrantTree(params) {
      ...
      if (this.exchangeState === 'normal') {
        ...
        getGrantTree({ [this.rightTreeParams]: params.guid }, 'Bill2ChgPro').then(res => {
          ...
          Bus.$emit('sendProjectListData', this.tableListData)
          // this.rightTreeExpandedNode = ['-1']
        })
      }
    },
    ...
    grantSave() {
      ...
      // Bus.$emit('sendCurrentRowGuid')
      this.$store.dispatch('SetLeftTreeNodeFamilyIdList', [this.selectedBillRowGuid])
      if (this.exchangeState === 'normal') {
        const params = {
        ...
        saveGrant(params, 'Bill2ChgPro').then(res => {
          this.$alert('授权成功！', {
            center: true,
          ...
          this.getProjectGrantTree({ guid: this.selectedBillRowGuid })
