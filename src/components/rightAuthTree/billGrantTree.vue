...
export default {
  ...
  created() {
    ...
    // 获取右侧树详情信息
    Bus.$off('getBillGrantTree')
    Bus.$on('getBillGrantTree', (params) => {
      this.getBillGrantTree(params)
    })
  ...
  methods: {
    ...
    getBillGrantTree(params) {
      ...
      if (this.exchangeState === 'normal') {
        ...
        getGrantTree({ [this.rightTreeParams]: params.guid }, 'Permit2Bill').then(res => {
          ...
          Bus.$emit('sendBillListData', this.tableListData.filter(item => item.isleaf === 1))
          Bus.$emit('sendBillInfoData', this.tableListData.filter(item => item.isleaf === 1))
    ...
    grantSave() {
      ...
      if (this.exchangeState === 'normal') {
        ...
        saveGrant(params, 'Permit2Bill').then(res => {
          this.$alert('授权成功！', {
            center: true,
          ...
          this.getBillGrantTree({ guid: this.selectedSubMenuGuid })
